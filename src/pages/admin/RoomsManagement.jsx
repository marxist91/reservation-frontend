...
function RoomsManagement() {
  ...
  const [viewMode, setViewMode] = useState('grid'); // 'table' ou 'grid'
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(12); // 12 pour la grille (3x4)
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      // Mettre à jour manuellement le cache avec les données retournées
      if (response.updated) {
        const currentRooms = queryClient.getQueryData(['rooms']) || [];
        
        // Créer un NOUVEAU tableau (référence différente) pour forcer le re-render
        const updatedRooms = [...currentRooms].map(room => 
          room.id === response.updated.id ? { ...response.updated } : { ...room }
        );
        
        // Remplacer directement les données du cache
